<template>
  ...
  <div class="max-w-5xl mx-auto px-4 py-6">
    ...
    >
      ...
      <NuxtLink
        to="/registerTeacher"
        class="bg-pink-400 text-black p-2 inline-block"
      >
        No accaunt? We have a proposition for you, create an account for a
        special price, just 14.99€, before IVA!
      </NuxtLink>
  ...
  try {
    ...
    if (res.ok && data.authenticated) {
      if (data.role === "teacher") {
        ...
        message.value = `Role mismatch: You are ${data.role}, not teacher`;
      }
    } else {
      message.value = `Login failed: ${data.error || "unknown error"}`;
    }
  } catch (err: any) {
