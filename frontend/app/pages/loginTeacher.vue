<template>
  <div class="bg-[#404040] w-full">
    <h1 class="text-[24px] text-white max-w-5xl mx-auto px-4 py-3">Inici de Sessió - Professors</h1>
  </div>
  <div class="max-w-5xl mx-auto px-4 py-6">
    <form class="bg-white min-h-screen flex flex-col" @submit.prevent="handleSubmit">
      ...
      <NuxtLink to="/registerTeacher" class="bg-pink-400 text-black p-2 inline-block mt-4 text-center">
        ¿No tienes cuenta? ¡Regístrate como profesor!
      </NuxtLink>
  ...
  try {
    ...
    if (res.ok && data.authenticated) {
      ...
      if (data.role === "teacher") {
        ...
        message.value = `❌ Error: Eres ${data.role}. Este login es solo para profesores. Por favor, usa el login correcto.`;
      }
    } else {
      errorMessage.value = true;
