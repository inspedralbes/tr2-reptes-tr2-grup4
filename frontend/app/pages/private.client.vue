<template>
  <div class="max-w-5xl mx-auto px-4 py-6 min-h-screen">
    ...
    <aside class="fixed left-4 top-24 w-48 border bg-white p-2">
      ...
      <button v-for="s in docVM.sections" :key="s.id" type="button"
    ...
    <section v-for="section in docVM.sections" :key="section.id" :id="`sec-${section.id}`"
      ...
      <div class="flex items-start justify-between gap-3">
        ...
        <div class="flex gap-2 shrink-0">
          <button v-if="editingId !== section.id" class="border rounded px-3 py-1" type="button"
            @click="startEdit(section)">
            Edit
          </button>

          <template v-else>
            <button class="border rounded px-3 py-1" type="button" :disabled="saving" @click="saveSection(section)">
              Save
            </button>
            <button class="border rounded px-3 py-1" type="button" :disabled="saving" @click="cancelEdit(section)">
              Cancel
            </button>
          </template>
        </div>
